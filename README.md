...
* LangGraph의 도입은 감정 기반 일지 생성에 있어 각 단계를 구조화하고 디버깅을 용이하게 했으며, 상태 기반 흐름 구성에 대한 이해를 높였음
...
* 현재는 실시간 WebSocket 기반보다는 **RESTful API 방식**으로 구현 중인데, 그 이유는 클라이언트가 게임이 아닌 운영 도구 목적에 가깝고, 각 요청에 대한 로그 및 결과 추적이 더 용이하기 때문임
