...
5. **디스코드 챗봇 3D 에셋 생성**
   ...
   * 디스코드 채팅에서 `!3d [프롬프트]` 명령어로 3D 오브젝트(GLB 파일)를 생성하고, 완성된 3D 모델을 채널에 자동 업로드할 수 있습니다. (2025.05.26 추가)
   * 동작 흐름:
   ...
   * 예시 사용법:
     ...
     !3d 작은 나무 의자
     ```
     → 3D 모델 생성 완료 시, GLB 파일이 채팅에 첨부되어 전송됩니다.
...
* **입력**: 디스코드 채팅 명령어 `!3d [프롬프트]` (예: `!3d 작은 나무 의자`)
* **프로세스 흐름**:
  1. 사용자가 디스코드 채팅에 3D 생성 명령어 입력
  2. 프롬프트를 내부 API(PROMPT_CONVERT_API)로 변환하여 영어 등 긍정 프롬프트로 변환
  3. 변환된 프롬프트로 이미지 생성 서버(MVADAPTER_SERVER)에서 PNG 텍스처 생성
  4. front/back/left 텍스처 이미지를 추출
  5. HY3D_SERVER에 텍스처 이미지를 업로드하여 GLB(3D 모델) 파일 생성
  6. 완성된 GLB 파일을 디스코드 채널에 자동 업로드
* **출력**: 3D GLB 파일이 디스코드 채널에 첨부되어 전송됨
* **예시**:
  ```
  !3d 작은 나무 의자
  ```
